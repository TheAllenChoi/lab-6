...
remove_outliers <- function(df, ..., sd_thresh = 3) {
  ...
  df |> 
    ...
    summarize(across(var_names, ~(.x - mean(.x)) / sd(.x))) |> 
    
    summarize(across(var_names, ~(.x > -sd_thresh & .x < sd_thresh)))
  
  # boolean mask original df
  
  
  # for each variable in ...
  # calculate mutate(paste0(..., z-val) = (... - mean(...) / sd(...) )
  
  # how do i get R to calculate z-value for each column, put it in separate column????
